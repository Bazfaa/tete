...
PUTIH = ('\x1b[1;97m')
# Banner
banner = (f"""     {MERAH}({PUTIH}★SELAMAT DATANG PARA PENGGUNA TERMUX★{MERAH}){PUTIH}
{MERAH}◖▓◣▁▁▁▁▁▁▁▁▁▁▁▁◢▓◗
{PUTIH}بِســـــــــــــــــــــــــــــمِﷲِ•الرَّحْمَنِ•ارَّحِيم
{MERAH}◖▓◤▔▔▔▔▔▔▔▔▔▔▔▔◥▓◗

{HIJAU} ←├M┤├u┤├h├B┤├a┤├s┤├a┤├r┤├i┤ →
   ...
   """)
    ...
    if masuk == '1' or masuk == '01':
      ...
      if 'c_user=' in str(cookie):
        ...
        exit(f"{MERAH}.-.{MERAH} Cookie Tidak Valid")#!/usr/bin/env python3
import requests, json, time, re, os
    ...
    try:
      with requests.Session() as r:
        ...
        if len(find) == 0:
          ...
          for y in find:
            response = r.get(f'https://web.facebook.com/adsmanager/manage/campaigns?act={y}&nav_source=no_referrer', headers = headers)#!/usr/bin/env python3
import requests, json, time, re, os
    ...
    try:
      with requests.Session() as r:
        ...
        if len(find) == 0:
          ...
          for y in find:
            response = r.get(f'https://web.facebook.com//manage/campaigns?act={y}&nav_source=no_referrer', headers = headers)
            token = re.search('(EAAB\w+)', response.text).group(1)
            print(f"\n{KUNING} {HIJAU} [Berhasil Mendapatkan Token] >{PUTIH} Token Kamu :{HIJAU} {token}")
    except Exception as e:
      exit(f"{MERAH}!.{MERAH} {e}")
...
if __name__=='__main__':
  os.system('git pull');convert()#!/usr/bin/env python3
import requests, json, time, re, os
